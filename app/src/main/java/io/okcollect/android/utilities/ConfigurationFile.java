package io.okcollect.android.utilities;
...
public class ConfigurationFile {
    ...
    public ConfigurationFile(final Context context, final String environment) {
        displayLog("ConfigurationFile called ");
        ConfigurationFile.context = context;
        dataProvider = new io.okcollect.android.database.DataProvider(context);
        ...
        try {
            if (environment.equalsIgnoreCase("PROD")) {
                try {
                    Parse.initialize(new Parse.Configuration.Builder(context)
                            .applicationId(io.okcollect.android.utilities.Constants.PROD_APPLICATION_ID)
                ...
                try {
                    Parse.initialize(new Parse.Configuration.Builder(context)
                            .applicationId(io.okcollect.android.utilities.Constants.DEVMASTER_APPLICATION_ID)
                            .clientKey(io.okcollect.android.utilities.Constants.DEVMASTER_CLIENT_ID)
                            .server("https://parseapi.back4app.com/").enableLocalDataStore().build());
                ...
                try {
                    Parse.initialize(new Parse.Configuration.Builder(context)
                            .applicationId(io.okcollect.android.utilities.Constants.SANDBOX_APPLICATION_ID)
            ...
            fetchAddresses("false", "false", phonenumber);
        } catch (Exception e4) {
            displayLog("parse initialize error " + e4.toString());
        }
                                                        ...
                                                        if (reallyKill) {
                                                            ...
                                                            try {
                                                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                                                    context.stopService(new Intent(context, io.okcollect.android.services.LocationService.class));
                                                                } else {
                                                                    context.stopService(new Intent(context, io.okcollect.android.services.LocationService.class));
                                                                }
                                                                stopPeriodicPing();
                                                                ...
                                                                if (noForeground.length() > 0) {
                                                                    ...
                                                                    if (noForeground.toLowerCase().contains(brand.toLowerCase())) {
                                                                        ...
                                                                        try {
                                                                            /*
                                                                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                                                                context.startForegroundService(new Intent(context, io.okcollect.android.services.LocationService.class));
                                                                            } else {
                                                                                context.startService(new Intent(context, io.okcollect.android.services.LocationService.class));
                                                                            }
                                                                            */
    ...
    private void decideWhatToStart() {
        /*
        io.okcollect.android.asynctask.GeofenceTask geofenceTask = new io.okcollect.android.asynctask.GeofenceTask(context);
    ...
    private void sendEvent(HashMap<String, String> parameters, HashMap<String, String> loans) {
        try {
            ...
            io.okcollect.android.utilities.OkAnalytics okAnalytics = new io.okcollect.android.utilities.OkAnalytics(context, environment);
        ...
        try {
            ...
            if (tempPhonenumber != null) {
                if (tempVerify) {
                    ...
                    io.okcollect.android.callback.GetAddressCallBack getAddressCallBack = new io.okcollect.android.callback.GetAddressCallBack() {
                        @Override
                        public void querycomplete(String response, boolean me) {
                            if (me) {
                                displayLog("good response " + response);
                    ...
                    io.okcollect.android.asynctask.GetAddressTask getAddressTask = new io.okcollect.android.asynctask.GetAddressTask(context, getAddressCallBack);
                    getAddressTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                }
            }
        } catch (Exception e) {
